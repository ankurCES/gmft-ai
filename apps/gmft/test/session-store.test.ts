...
describe('SessionStore', () => {
  ...
  it('append() writes to the current session log', async () => {
    ...
    await store.append({ role: 'user', content: 'hello', ts: 1000 });
    await store.append({ role: 'assistant', content: 'hi', ts: 1001 });
  ...
  it('append() throws when there is no current session', async () => {
    ...
    await expect(
      store.append({ role: 'user', content: 'orphan', ts: 1 }),
  ...
  it('list() returns sessions sorted by mtime desc, with current flag', async () => {
    ...
    await store.append({ role: 'user', content: 'a-1', ts: 1 });
    await new Promise((r) => setTimeout(r, 20));
    const b = await store.start('beta');
    await new Promise((r) => setTimeout(r, 20));
    await store.append({ role: 'user', content: 'b-1', ts: 1 });
    await store.append({ role: 'assistant', content: 'b-2', ts: 2 });
